...
def nonParamAttention(y_hat, x, x_hat, device='cpu'):
    '''
    x is the true input
...
def paramAttention(y_hat, x, x_hat, w, device):
    '''
    x is the true input
    x_hat is the train or sample input
    y_hat is the train or sample output
...
def train(y_hat, x_hat, lr = 50, epoches = 1000, device = 'cpu'):
    w = torch.ones((x_hat.shape[0], x_hat.shape[0]), dtype=torch.float32, requires_grad = True, device = device)
    x_hat = x_hat.to(device)
    y_hat = y_hat.to(device)
    for _ in range(epoches):
        results = paramAttention(y_hat, x_hat, x_hat, w, device)
...
if __name__ == "__main__":
    ...
    plt.plot(X, nonParamAttention(y_train, X, x_hat).expand(y_train.shape).cpu(), label = 'Non-Param Predict')
    
    w = train(y_train, x_hat, lr = 50, epoches = epoches, device = device)
    print(w)
    # print(paramAttention(y_train, X, x_hat, w))
    plt.plot(X, paramAttention(y_train, X, x_hat, w, device = device).cpu().detach(), label = 'Param Predict')
    plt.legend()
    plt.show()